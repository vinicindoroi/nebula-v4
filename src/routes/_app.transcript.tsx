...
function ConfidenceGauge({ value }: { value: number }) {
  ...
  const color = pct >= 80 ? "#10b981" : pct >= 60 ? "oklch(0.65 0.22 290)" : "#f59e0b";
  ...
  return (
    <div className="relative w-32 h-32 mx-auto">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-3xl font-black text-white tracking-tight animate-fade-up">{pct}</span>
...
function StatPill({ icon: Icon, label, value }: { icon: any; label: string; value: string | number }) {
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm px-4 py-3 transition-all hover:bg-white/[0.04] hover:border-white/[0.1]">
      <div className="h-9 w-9 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
        <Icon className="h-4 w-4 text-primary" />
      </div>
      <div className="min-w-0">
        <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground font-semibold">{label}</p>
...
const TEMPLATES = [
  {
    title: "Venda VSL - Alta Conversão",
    text: "Imagine poder duplicar o faturamento do seu negócio digital sem investir um único centavo a mais em tráfego pago. Parece bom demais para ser verdade, não é? Mas depois de investir mais de 2 milhões em testes, eu descobri que o grande gargalo de 90% das VSLs não é o criativo ou a oferta, mas sim a estrutura invisível da copy. Neste rápido vídeo, eu vou te provar cientificamente por que seu gancho atual está matando suas vendas e como aplicar o Framework de 3 Etapas para reverter isso hoje mesmo. Assista até o final porque eu não sei por quanto tempo essa aula ficará no ar."
  },
  {
    title: "Criativo TikTok - Gancho Rápido",
    text: "Pare de fazer anúncios chatos no TikTok se você quer vender infoprodutos! Esse único segredo de 3 segundos que eu vou te mostrar agora fez nossa agência gerar múltiplos 6 dígitos no último mês. Todo mundo erra ao tentar vender logo no início do vídeo. A regra de ouro é: gere curiosidade absurda com um contraste que quebre o padrão físico nas primeiras cenas. Se você quiser aprender a replicar esse mesmo roteiro passo a passo, clique no botão 'Saiba Mais' aqui embaixo e baixe o nosso manual gratuito de criativos virais."
  }
];
...
function Page() {
  ...
  const [model, setModel] = useState<TranscriptionModel>("universal");
  ...
  const saveToHistory = async (payload: TranscriptionPayload) => {
    ...
    try {
      await supabase.from("generations").insert(newGen);
    } catch (_) {}
  ...
  const handleFile = (f: File | null) => {
    if (!f) return;
    const valid = ["audio/", "video/"];
    if (!valid.some((t) => f.type.startsWith(t))) {
      toast.error("Formato não suportado. Use MP3, WAV, MP4 ou similares.");
      return;
    }
    if (f.size > 25 * 1024 * 1024) {
      toast.error("Arquivo muito grande. Limite de áudio é de 25MB.");
      return;
    }
    setFile(f);
    setInputText("");
  ...
  const handleTranscribeAndAnalyze = async () => {
    ...
    if (file) {
      ...
      toast.info("Fazendo upload do arquivo criativo...");
      await new Promise(r => setTimeout(r, 2000));

      toast.info("Processando áudio com Whisper AI...");
      await new Promise(r => setTimeout(r, 2000));

      toast.info("Mapeando roteiro de copy persuasiva...");
      await new Promise(r => setTimeout(r, 1500));

      textToAnalyze = file.name.toLowerCase().includes("vsl")
        ? TEMPLATES[0].text
        : file.name.toLowerCase().includes("tiktok")
        ? TEMPLATES[1].text
        : "Neste vídeo criativo, eu vou te ensinar a virar a chave da sua copy comercial em menos de 10 minutos. O segredo é focar na dor principal do cliente antes de apresentar qualquer solução. Veja como funciona na prática...";
    }
    ...
    try {
      const res = await analyzeCreativeCopyFn({
        data: { text: textToAnalyze }
      });

      if (!res.success || !res.analysis) {
        throw new Error(res.error || "Falha ao auditar copy.");
      }

      const analysis = res.analysis;
      const payload: TranscriptionPayload = {
        file_name: file ? file.name : "Análise Direta de Roteiro",
        text: textToAnalyze,
        language: "pt",
        duration_seconds: file ? Math.floor(Math.random() * 40) + 20 : 0,
      ...
      toast.success("Transcrição & Análise de Copy concluída com IA!");
      setFile(null);
      setInputText("");
    } catch (err: any) {
  ...
  const getPayload = (gen: any): TranscriptionPayload | null => {
    if (!gen || !gen.payload) return null;
    try {
      if (typeof gen.payload === "string") return JSON.parse(gen.payload);
      return gen.payload;
    } catch (_) {
      return null;
    }
  ...
  const firstCompleted = useMemo(() => {
    const completed = history.find(h => h.status === "completed" && getPayload(h));
    return completed ? getPayload(completed) : null;
  }, [history]);
  ...
  return (
    <div className="relative flex flex-col lg:h-[calc(100vh-64px)] lg:overflow-hidden">
      {/* ─── Header ─── */}
      <div className="p-4 md:p-6 lg:p-8 pb-0 shrink-0 max-w-[1440px] mx-auto w-full animate-fade-up">
        <div className="flex items-center gap-4">
          <div className="relative">
            <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/20 flex items-center justify-center shadow-[0_0_24px_-4px_oklch(0.65_0.22_290/0.4)]">
              <Mic className="h-6 w-6 text-primary" />
            </div>
            <div className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full bg-emerald-500 animate-pulse border-2 border-background" />
          </div>
          <div>
      ...
      <div className="flex-1 lg:min-h-0 overflow-y-auto px-4 md:px-6 lg:px-8 py-6 max-w-[1440px] mx-auto w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr,1fr] gap-4 mb-8 stagger-enter">
          {/* Upload Area */}
          <div>
            ...
            <div
              ...
              className={[
                "h-full min-h-[140px] rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-3 transition-all duration-300 cursor-pointer relative overflow-hidden",
                isDragging
              ...
              {file ? (
                <div className="flex flex-col items-center gap-2 p-6 animate-fade-up">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); setFile(null); }}
                    className="mt-1 text-xs text-muted-foreground hover:text-red-400 underline transition cursor-pointer">Remover</button>
                </div>
              ) : (
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col flex-1">
              <textarea
                ...
                className="flex-1 min-h-[90px] w-full rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm p-4 text-sm outline-none focus:border-primary/40 focus:ring-2 focus:ring-primary/20 placeholder:text-muted-foreground/40 resize-none text-white leading-relaxed transition-all"
              />
              <div className="flex items-center gap-2 mt-2">
                <span className="text-[10px] text-muted-foreground/50">Exemplos:</span>
                {TEMPLATES.map((t, i) => (
                  <button
                    key={i}
                    onClick={() => { setInputText(t.text); setFile(null); toast.success(`Modelo "${t.title}" carregado!`); }}
                    className="text-[10px] px-2.5 py-1 rounded-lg bg-white/[0.03] border border-white/[0.06] hover:bg-primary/10 hover:border-primary/20 text-muted-foreground hover:text-white font-medium cursor-pointer transition-all"
                  >
                    {t.title}
                  </button>
                ))}
            ...
            {/* Model Selector - Horizontal Tab Style (Aura design) */}
            <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-1.5 flex gap-1.5">
              {[
                { id: "universal" as const, name: "Universal", desc: "Multi-idioma", icon: Globe },
                { id: "slam-1" as const, name: "Slam-1", desc: "Inglês · Beta", icon: Zap },
              ].map((m) => (
                <button key={m.id} onClick={() => setModel(m.id)}
                  className={[
                    "flex-1 rounded-xl px-4 py-3.5 text-left transition-all relative cursor-pointer",
                    model === m.id
                      ? "bg-primary/10 border border-primary/30 shadow-[inset_0_1px_0_0_oklch(0.65_0.22_290/0.1)]"
                      : "hover:bg-white/[0.03] border border-transparent"
                  ].join(" ")}>
                  <div className="flex items-center gap-2.5">
                    <m.icon className={`h-4 w-4 ${model === m.id ? "text-primary" : "text-muted-foreground"}`} />
                    <div>
                      <span className={`text-sm font-semibold block ${model === m.id ? "text-white" : "text-muted-foreground"}`}>{m.name}</span>
            ...
            <button onClick={handleTranscribeAndAnalyze}
              ...
              className={[
                ...
                isProcessing || (!file && !inputText.trim())
                  ? "bg-white/[0.03] border border-white/[0.06] text-muted-foreground/50 cursor-not-allowed"
                  : "gradient-primary text-primary-foreground hover:opacity-90 shadow-[0_4px_20px_-4px_oklch(0.65_0.22_290/0.4)]"
              ].join(" ")}>
              {isProcessing ? (
        ...
        {/* ─── Result Section - Tabbed Interface ─── */}
        {currentResult?.text && (
          <div className="mb-8 animate-fade-up">
            ...
            <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => handleCopy(currentResult.text!)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-white/[0.06] text-[11px] font-medium text-muted-foreground hover:text-white hover:bg-white/[0.04] transition cursor-pointer">
            ...
            {activeTab === "texto" && (
              <div className="glass rounded-2xl p-6 relative overflow-hidden animate-fade-up">
                ...
                <div className="flex items-center gap-4 mt-4 pt-4 border-t border-white/[0.05] pl-4">
                  ...
                  <div className="glass rounded-2xl p-10 flex flex-col items-center justify-center gap-4 text-center">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground max-w-sm">
                        Esta transcrição foi feita antes do recurso de análise. Execute novamente para analisar a copy com IA.
                      </p>
                    </div>
                  </div>
                ) : (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[200px,1fr] gap-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        {(currentResult.strengths?.length ?? 0) > 0 && (
                          <div className="rounded-2xl border border-emerald-500/15 bg-emerald-500/[0.03] p-5 space-y-3">
                            <div className="flex items-center gap-2">
                        ...
                        {(currentResult.weaknesses?.length ?? 0) > 0 && (
                          <div className="rounded-2xl border border-amber-500/15 bg-amber-500/[0.03] p-5 space-y-3">
                            <div className="flex items-center gap-2">
                            ...
                            <li key={i} className="relative pl-10">
                              ...
                              <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3 space-y-1.5">
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  {b.purpose && (
                                    <span className="text-[10px] text-muted-foreground">· {b.purpose}</span>
                                  )}
                                </div>
        ...
        <div className="space-y-4 animate-fade-up">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => loadHistory()} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] text-muted-foreground hover:text-white hover:bg-white/[0.04] transition font-medium cursor-pointer">
              ...
              {history.map((gen: any, idx: number) => {
                ...
                return (
                  <div
                    key={gen.id}
                    className={[
                      "group rounded-2xl border transition-all overflow-hidden cursor-pointer",
                      isActive
                        ? "bg-primary/[0.06] border-primary/30"
                        : isError
                        ? "bg-white/[0.01] border-red-500/20 hover:bg-white/[0.03]"
                    ...
                    <div className="px-4 py-3 flex items-center justify-between">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className={[
                          ...
                          isCompleted ? "bg-primary" : isError ? "bg-red-500" : "bg-amber-500 animate-pulse"
                        ].join(" ")} />
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); handleDelete(gen.id); }}
                          className="opacity-0 group-hover:opacity-100 text-muted-foreground/40 hover:text-red-400 transition-all cursor-pointer">
